...
import { BundleMarkdown, FileBundleData, ContextModule } from './bundleMarkdown';
...
export async function generateBundle(
    ...
    // -----------------------------
    // Collect data for large file analysis
    const filesForAnalysis: Array<{ path: string; content: string; extension: string }> = [];
    ...
    for (const fileData of fileDataList) {
        ...
        if (fileData.isSkipped || !fileData.content) {
            ...
            // Collect for large file analysis
            filesForAnalysis.push({
                path: fileData.relativePath,
                content: fileData.content,
                extension: path.extname(fileData.uri.fsPath).toLowerCase()
            });
        }
    ...
    // -----------------------------
    // Analyze largest files for transparency
    const largestFiles = TokenStats.findLargestFiles(filesForAnalysis, 5);
    const largeFilesSection = TokenStats.formatLargeFiles(largestFiles);
    ...
    // Add largest files info if there are significant files (>10k tokens)
    if (largeFilesSection && largestFiles.length > 0 && largestFiles[0].tokens > 10000) {
        header += `\n${t('largeFiles') || 'Top heavy files'}:\n${largeFilesSection}\n`;
    }
...
/**
 * Generate Markdown bundle using unified format (v0.2.7)
 * 
 * This new format provides:
 * - Consistent Markdown structure across all presets
 * - Emoji markers for preset identification
 * - Top Heavy Files table for token transparency
 * - Syntax-highlighted code blocks
 * - Foundation for context modules (Database, Infra, API)
 */
export async function generateMarkdownBundle(
    ...
    preset: PresetType,
    smartTree: boolean,
    excludedFolderPaths: Set<string> = new Set(),
    binaryFilePaths: Set<string> = new Set()
): Promise<string> {
    const config = vscode.workspace.getConfiguration('projectBundler');
    const binaryExtensions = new Set(
        (config.get<string[]>('binaryExtensions', [])).map(e => e.toLowerCase())
    ...
    // -----------------------------
    // 1. Read files and collect data
    // -----------------------------
    const fileDataList: FileBundleData[] = [];
    ...
    // -----------------------------
    // 2. Extract database schema as context module
    // -----------------------------
    ...
    if (dbExtractor && sqlMigrationFiles.length > 0) {
        ...
        if (dbResult.tables.length > 0) {
            contextModules.push({
                type: 'DatabaseSchema',
                title: '🗄️ Database Schema (from Migrations)',
                content: dbResult.transformed,
                tokenCount: dbResult.tokenCount,
                metadata: {
                    source: 'SQL Migrations',
                    tables: dbResult.tables.length
                }
            });
        }
    }
    ...
    if (dbExtractor && prismaFiles.length > 0) {
        for (const prismaFile of prismaFiles) {
            ...
            if (dbResult.tables.length > 0) {
                contextModules.push({
                    type: 'DatabaseSchema',
                    title: '🗄️ Database Schema (from Prisma)',
                    content: dbResult.transformed,
                    tokenCount: dbResult.tokenCount,
                    metadata: {
                        source: 'Prisma Schema',
                        tables: dbResult.tables.length,
                        file: path.relative(rootPath, prismaFile.fsPath)
                    }
                });
            }
        }
    ...
    // -----------------------------
    // 3. Generate Markdown bundle
    // -----------------------------
    const mdGenerator = new BundleMarkdown({
        rootPath,
        preset,
        files: fileDataList,
        contextModules: contextModules.length > 0 ? contextModules : undefined,
        includeStructure: true,
        includeContents: true,
        includeMermaid: preset === PresetType.Architecture,
        smartCompression: smartTree,
